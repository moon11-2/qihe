...
def normalize_review_result(data: dict[str, Any], source: ContractSource, source_text: str = "") -> ReviewResult:
    ...
    clause_reviews = _normalize_clause_reviews(data.get("clause_reviews") or data.get("risk_items"))
...
def _normalize_clause_reviews(value: Any) -> list[ClauseReview]:
        ...
        reviews.append(
            ClauseReview(
                risk_title=text_or_default(
                    safe_output_text(item.get("risk_title") or item.get("title")),
                    f"风险事项 {index}",
                ),
                risk_level=risk_level(item.get("risk_level") or item.get("level")),
                clause=text_or_none(item.get("clause") or item.get("涉及条款")),
                risk_analysis=text_or_default(
                    safe_output_text(item.get("risk_analysis") or item.get("description") or item.get("risk")),
                    "该事项需要结合完整合同文本进一步确认。",
                ),
                ...
                suggested_replacement=text_or_none(
                    safe_output_text(item.get("suggested_replacement") or item.get("replacement"))
                ),
                legal_basis=safe_string_list(item.get("legal_basis")) or DEFAULT_LEGAL_BASIS,
            )
        )
