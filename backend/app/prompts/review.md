...
9. 只有已明确完成审查且确实未发现明显风险时，才可返回空风险数组：`summary` 必须明确使用“未发现明显风险”，`risk_level` 必须为现有的 `低风险`，`score` 必须在 90-100，`clause_reviews` 返回空数组 `[]`；不得使用“合同绝对安全”或其他绝对安全表述。
10. 除上述严格无风险情形外，`clause_reviews` 必须包含至少一个完整、具体且可规范化的风险对象；不得用空对象、字符串、`null` 或缺少风险标题/分析的占位元素充数。
